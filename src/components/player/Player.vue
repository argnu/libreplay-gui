  ...
  <div>
    <div ref="player" style="max-height:300px;border: 0.5px solid grey;">
      <div class="button-bar" style="width:100%">
            ...
            <i title="Play/Pause" :class="{ 'fa-pause': status.state == 'play', 'fa-play': status.state == 'stop' || status.state == 'pause' }"
                      class="fa" @click="togglePause()"></i>
...
export default {
  ...
  methods: {
    chgTime: function() {
      axios.post('http://localhost:3000/command', {command: 'seekcur', params: [ this.status.duration * this.time / 100 ]})
    },
    ...
    playNext: function() {
      axios.post('http://localhost:3000/command', {command: 'next'})
            .then(r => {
              this.refresh();
            })
    },
    ...
    playPrev: function() {
      axios.post('http://localhost:3000/command', {command: 'previous'})
      .then(r => {
        this.refresh();
      })
    },
    ...
    togglePause: function() {
      ...
      axios.post('http://localhost:3000/command', {command: command})
            .then(r => {
              this.refresh();
            });
    },
    ...
    stop: function() {
      axios.post('http://localhost:3000/command', {command: 'stop'})
            .then(r => {
              this.refresh();
            });
    },
    ...
    toggleRepeat: function() {
      ...
      axios.post('http://localhost:3000/command', {command: 'repeat', params: [repeat_state]})
          .then(r => {
            this.refresh();
          })
    },
    ...
    toggleRandom: function() {
      ...
      axios.post('http://localhost:3000/command', {command: 'random', params: [random_state]})
          .then(r => {
            this.refresh();
          })
    },
    ...
    clear: function() {
      axios.post('http://localhost:3000/command', {command: 'clear'})
            .then(r => {
              this.refresh();
            });
    },
    ...
    removeSong: function(i) {
      axios.post('http://localhost:3000/command', {command: 'delete', params: [i]})
           .then(r => {
             this.refresh();
           })
    },
    ...
    addSong: function(song) {
      axios.post('http://localhost:3000/command', {command: 'add', params: [song.file]})
           .then(r => {
             this.refresh();
           })
    ...
    addAndPlay(song) {
      axios.post('http://localhost:3000/playlist/add', { uri: song.file })
           .then(r => {
              return axios.post('http://localhost:3000/command', {command: 'playid', params: [ r.data.Id ]})
           })
           .then(r => this.refresh())
           .catch(e => console.error(e));
    },
    ...
    playSong(song) {
      return axios.post('http://localhost:3000/command', {command: 'playid', params: [ song.Id ]})
          .then(r => {
              this.refresh();
          })
    }
