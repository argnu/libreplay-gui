...
export function getArtists() {
  return axios.get('http://localhost/mpd/rest/artists', {responseType: 'json'})
    .then( response => {
...
export function getAlbums(q) {
  return axios.get(`http://localhost/mpd/rest/albums?${q || ''}`, {responseType: 'json'})
    .then( response => {
        return Promise.resolve(response.data);
      })
    .catch(e => handleError(e));
...
export function getSongs(q) {
  return axios.get(`http://localhost/mpd/rest/songs?${q || ''}`, {responseType: 'json'})
    .then( response => {
        return Promise.resolve(response.data);
      })
    .catch(e => handleError(e));
